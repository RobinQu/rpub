...
(function() {
  ...
  Publisher.prototype.diff = function (manifest, cb) {
    var existing, added, revoked, updated, diff, i, len, name, uuid, uuids;
    //quick diff
    if(!manifest) {
  ...
  Publisher.prototype.removeIssue = function (uuid, cb) {
    var issue, that;
  ...
  Publisher.prototype.updateIssue = function (issue, file, cb) {
    var that, k, obj;
    if(typeof issue === "string") {
      obj = this.issues[uuid];
    } else {//update meta
      obj = Object.create(this.issues[issue.uuid]);
      if(issue.meta) {
        for(k in issue.meta) {
    ...
    cb = cb || noop;
    that = this;
    this.storage.update(issue, file, function(e, data) {
      if(e) {
        return cb(e);
      }
      that.issues[data.uuid] = obj;
      cb(null);
    });
