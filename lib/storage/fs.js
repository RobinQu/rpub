(function() {
  ...
  FSStorage.prototype.compact = function (manifest, cb) {
    var list, i, len, compacted, name, uuid, meta, files, issue;
    ...
    for(i=0, len=list.length; i<len; i++) {
      ...
      try {
        if(fs.existsSync(path.join(this.metaDir, name))) {
          meta = JSON.parse(fs.readFileSync(path.join(this.metaDir, name)));
        }
      } catch (e) {
