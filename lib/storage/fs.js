(function() {
  ...
  var FSStorage = module.exports = function(opt, cb) {
    this.dir = opt.dir;
  ...
  FSStorage.prototype.destroy = function (issueOrUUID, cb) {
    ...
    child_process.exec("rm -rf " + that._dirpath(uuid), function(e) {
      if(e) {
        return cb(e);
      }
  ...
  FSStorage.prototype.compact = function (manifest, cb) {
    ...
    list = list.filter(function(dirname) {
      return dirname[0] !== ".";
    });
    compacted = {};
    for(i=0, len=list.length; i<len; i++) {
      ...
      if(manifest[uuid]) {//inherit other properties in given manifest
        issue = manifest[uuid];
        issue.uuid = uuid;
      } else {//create new one
        issue = {
